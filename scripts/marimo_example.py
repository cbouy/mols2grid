...
def display_selection(mo, mol_to_svg, results, sel_ids):
    # This cell displays the selected molecules in a Marimo table.
    # The `mol_to_svg` function is used to render the molecule images directly in the table.
    # We filter the dataframe based on the selection state (`sel_ids`) from the grid above.
    ...
    table = mo.ui.table(
        ...
        format_mapping={
            "mol": mol_to_svg
            # "mol": lambda mol: mol_to_svg(mol, opts=opts) # If you want to use custom drawing options
        },
